'''
和prediction差不多，将test的text数据加载到数据迭代器，送到预训练模型里面将得到的输出结果保存到一个excel文件中
'''
import pandas as pd
import torch
...
def set_test(test_iter, model_file):
    ...
    dict={"id": [x for x in range (1,1413)],"label":pred_label_list[:1412]}
    result=pd.DataFrame(dict,)
    result.to_csv(config.base_dir+"result.csv",encoding="utf8")
    # print(pred_label_list)
    # print(true_label_list)

    # test_result_pd = pd.read_excel(config.base_dir + 'test.xlsx', encoding='utf8')
    # true_list = test_result_pd['num_label'].tolist()
    # print(len(true_list))

    # test_result_pd['pred'] = pred_label_list[:1412]
    # print(test_result_pd['pred'])
    #
