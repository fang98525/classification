...
config_.gpuid_str=",".join(str(i) for i in config_.gpuid)
# print("gpu id 为：",config_.gpuid)
# gpu_id = 1
os.environ["CUDA_VISIBLE_DEVICES"] ="1,2"#config_.gpuid_str
result_data_dir = Config().result_file
print('GPU ID: ', str(config_.gpuid_str))
...
def train(train_iter, test_iter, config):
    ...
    if n_gpu > 1:
        print("进行多gpu分布式训练")
        model = torch.nn.DataParallel(model)  # net = nn.DataParallel(net, device_ids=device_ids)
    ...
    out_dir = os.path.abspath(
        os.path.join(config.save_model, "runs_" + str(config_.gpuid), timestamp))
...
def set_test(model, test_iter):
    ...
    with torch.no_grad():
        ...
        # print("micro的F1, P, R分别为："+'model_{:.4f}_{:.4f}_{:.4f}_{}.bin'.format(F1, P, R))
        return F1, P, R
...
if __name__ == '__main__':
    ...
    train_iter = DataIterator(config.batch_size,
                              data_file=config.base_dir + 'train_enforce1.xlsx',
                              use_bert=config.use_bert,
                              tokenizer=tokenizer, seq_length=config.sequence_length)
    dev_iter = DataIterator(config.batch_size, data_file=config.base_dir + 'dev1.xlsx',
                            use_bert=config.use_bert,
